...
const Sign = () => {
  ...
  const checkOverlapEmail = () => {
    const findEmail = emailData.filter(
      (email) => email === check_email.current.value
    );
    return findEmail;
  };
  ...
  const onSubmit = async (evt) => {
    ...
    /* 
        ...
        */
    if (
      isEmpty(check_name.current.value) &&
      !regex.name.test(check_name.current.value)
    ) {
    ...
    if (
      isEmpty(check_email.current.value) &&
      !regex.email.test(check_email.current.value)
    ) {
    ...
    if (
      isEmpty(check_password.current.value) &&
      !regex.password.test(check_password.current.value)
    ) {
      alert(
        "8자리 이상의 영문, 숫자, 특수문자가 반드시 1개라도 포함되어야 합니다."
      );
    ...
    if (
      ...
      vaildEmail &&
      vaildPwd &&
      check_password.current.value === check_rePwd.current.value
    ) {
      try {
